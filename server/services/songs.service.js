...
class SongService {
  ...
  async getSongById (id) {
    return await this.collection.findOne({ id: { $eq: Number(id) } });
  }
  ...
  async updateSongLike (id) {
    ...
    try {
      ...
      const filter = { id };
      const update = { "$set": { liked: !song.liked } };
  ...
  async search (substring, exact) {
    const caseSensitive = exact ? "" : "i";
    const filter = [{ name: { $regex: `${substring}`, $options: `${caseSensitive}` } },
      { artist: { $regex: `${substring}`, $options: `${caseSensitive}` } },
      { genre: { $regex: `${substring}`, $options: `${caseSensitive}` } }
    ];
